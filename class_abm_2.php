...
class class_abm
{
	 ...
	 */
	public $textoPreguntarBorrar = "øConfirma que desea borrar el elemento seleccionado?";
...
<script>vex.defaultOptions.className = 'vex-theme-os'</script>";
	...
	private function generarFormModificacion($id, $titulo = "")
	{
		...
		foreach ($this->campo as &$campo)
		{
			...
			if ($campo->existeDato ('joinTable') and (!$campo->existeDato ('omitirJoin') or $campo->isOmitirJoin () == false))
			{
				...
				// NÔøΩtese el uso de ===. Puesto que == simple no funcionarÔøΩ como se espera
				// porque la posiciÔøΩn de 'a' estÔøΩ en el 1ÔøΩ (primer) caracter.
				if ($pos === false)
				{
		...
		{
			$camposSeleêEÖJ:V  êEÖJ:V                  êæ√I:V          ƒI:V  ¯EÖJ:V          ∞EÖJ:V   @      ∞EÖJ:V          rtirIdMultiple ($this->campoId, $this->tabla);
	...
	private function armar_fila_listado($fila, &$rallado)
	{
		...
		if ($this->mostrarEditar)
		{
			...
			// echo "<td class='celdaEditar'>" . $this->iconoEditar . $fila['ID'] . "</td> \n"êEÖJ:V  êEÖJ:V                  êæ√I:V          ƒI:V  ¯EÖJ:V          ∞EÖJ:V   @      ∞EÖJ:V          tar, $_SERVER['PHP_SELF'] . "?abm_editar=" . $fila['ID'] . $qsamb) . "</td> \n";
		...
		foreach ($tablas as $tabla)
		{
			...
			foreach ($this->campo as &$campo)
			{
				...
				if ($campo->getJoinTable () == $tabla or ($campo instanceof Campos_dbCombo))
				{
					...
					êEÖJ:V  êEÖJ:V                  êæ√I:V          ƒI:V  ¯EÖJ:V          ∞EÖJ:V   @      ∞EÖJ:V          Archivo']) and $campo['tipoArchivo'] != "")
							// {
