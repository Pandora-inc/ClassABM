 ...
 *--------------------------------------------------------------------
 * $Id: BCGupce.barcode.php,v 1.16 2009/11/09 04:08:41 jsgoupil Exp $
 *--------------------------------------------------------------------
...
class BCGupce extends BCGBarcode1D
{
		...
		if ($error_stop === false)
		{
			...
			if ($error_stop === false)
			{
				if ($c !== 6)
				{
					...
					$upce = '';
					if (substr ($this->text, 3, 3) === '000' || substr ($this->text, 3, 3) === '100' || substr ($this->text, 3, 3) === '200')
					...
					{ // No zero leading at manufacturer code
						if (substr ($this->text, 6, 4) === '0000' && intval (substr ($this->text, 10, 1)) >= 5 && intval (substr ($this->text, 10, 1)) <= 9)
	...
	public function getMaxSize()
	{
		...
		return array (
				$p[0] + $startlength + $centerlength + $textlength + $endlength + $lastcharlength,
				$p[1]
		);
	}
