...
class files {
    ...
    public function __construct($db, $IDFORMULARIO_pertenece = NULL, $pre = NULL) {
        ...
        if ($IDFORMULARIO_pertenece != null && $IDFORMULARIO_pertenece != '' && $pre == NULL) 
        {
            ...
            $query = 'SELECT documento.documen.* , documento.archivo.* ,'
                    . ' to_CHAR(documento.archivo.FECHAALTA, \'yyyy\') AS carpeta_server'
                    . ' FROM  documento.documen JOIN documento.archivo '
                    . 'ON documento.documen.IDDOCUMEN =documento.archivo.IDDOCUMEN '
                    . 'WHERE documento.documen.IDDOCUMEN = :iddocumen';
        ...
        else if($IDFORMULARIO_pertenece != null && $IDFORMULARIO_pertenece != '' && $pre != NULL) 
        {
            ...
            $query = 'SELECT documento.predocumen.* , documento.prearchivo.* ,'
                    . ' to_CHAR(documento.prearchivo.FECHAALTA, \'yyyy\') AS carpeta_server'
                    . ' FROM  documento.predocumen JOIN documento.prearchivo '
                    . 'ON documento.predocumen.IDDOCUMEN = documento.prearchivo.IDDOCUMEN '
                    . 'WHERE documento.predocumen.IDDOCUMEN = :iddocumen';
    ...
    /**
     * 
     * @param $imagen array Array con los datos de la imagen , solo 1 imagen 
     * @param string $path path donde se v a subir la img
     * 
     ...
     * 
     * @param array  $imagen-->array con dqatos de la img
     * @param string $path-->ruta
     * @param int    $tablacodumento-->puede ser documento o predocumento
     * @param int    $tablaarchivo-->puede ser archivo o prearchivo
     * @param int    $person-->person de quien sube la img
     * 
     * @return boolean
     */
     ...
     * checkDir
     * 
     * @param string $ruta_dir ruta de la carpeta que quiero chequear
     * @param number $crear si esta en cero solo chequea si esta en uno chequea si existe y si 
     ...
     * @param type $id_doc-->id de la tabla documento.documentos
     * @param type $nombrearchivo-->nombre real del archivo
     * @param type $person-->person de quien subio el archivo
     * @return bool
     */
